...
O histórico conserva hashes de entrada/saída e, em sobrescrita, a raiz efetiva e o caminho relativo do payload realmente gravado, com `compression: gzip` para novos backups e `compression: none` para backups legados raw. Essa raiz histórica é imutável e autoritativa para descoberta/restauração; a configuração atual nunca a substitui. No modo `.min`, a ausência de payload de backup é explícita. Estado e manifesto são comprovados antes da criação exclusiva do histórico; o journal só conclui depois de comprovar também o hash desse registro. `SelfMinifier-Tag` não está implementado. O manifesto de backup evolui para `formatVersion: 2`; estado, journal, journal de restauração e `Dados\Historico` permanecem em `formatVersion: 1`.
