...
import { collectProjectEntries, collectSourceEntries, isTechnicalExclusion, normalizeAbsolutePath } from './filesystem.js';
...
async function scanLegacy(configuration, options = {}) {
...
export async function scan(configuration, options = {}) {
  if (configuration?.schemaVersion === 2 || Object.hasOwn(configuration ?? {}, 'projectRoot')) return scanV2(configuration, options);
  return scanLegacy(configuration, options);
}
