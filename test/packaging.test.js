...
test('nomes de artefato derivam da versão e allowlist contém somente runtime necessário', async () => {
  ...
  assert.equal(metadata.packageName, 'SelfMinifier-0.3.0');
  assert.match(metadata.zipPath, /SelfMinifier-0\.3\.0\.zip$/);
...
test('pacote isolado resolve versão e inicia fora do repositório em caminho com espaços', async () => {
  ...
  try {
    ...
    if (/^Restricted$/i.test(hostPolicy)) {
      ...
      assert.equal((cmdStartup.stdout.match(/SELFMINIFIER v0\.3\.0\b/g) ?? []).length, 1, `${cmdStartup.stdout}\n${cmdStartup.stderr}`);
    }
  ...
  assert.equal(JSON.parse(request.stdout).version, '0.3.0');
    ...
    assert.equal((startup.stdout.match(/SELFMINIFIER v0\.3\.0\b/g) ?? []).length, 1);
...
test('limpeza fora de dist ou com nome inesperado é rejeitada', async () => {
  assert.throws(() => assertSafeDistTarget(projectRoot, join(projectRoot, 'src'), 'SelfMinifier-0.3.0'));
  assert.throws(() => assertSafeDistTarget(projectRoot, join(projectRoot, 'dist', 'outro'), 'SelfMinifier-0.3.0'));
});
